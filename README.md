...
2. **设定剧情**：房主创建角色池（每个角色可写一句描述），**可选**地选定自己扮演的角色（不选则以主持身份旁观，不参与行动）；再输入剧情设定，AI 会生成一个标题。
3. **加入**：其他玩家选择「加入房间」，输入名字和邀请码，然后认领一个尚未被认领的角色；也可以选择**旁观**（不认领角色，只观看剧情和聊天）。未被认领的角色由 AI 扮演。
...
7. **关闭 / 退出**：房主可点击「关闭房间」解散房间；任何人都可点击「退出房间」只是离开、房间继续运行，之后凭邀请码重新加入。房间不会因为无人而自动删除，会一直保留到被手动关闭或服务器重启。
