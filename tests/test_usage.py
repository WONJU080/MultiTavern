...
def test_engine_reports_semantic_failure_and_retains_cost_on_retry(tmp_path, monkeypatch):
    ...
    async def run():
        ...
        engine.pending_resolution = {
            "actions": {"id": "Wait"},
            "participants": {"id": ("Borin", "Borin", False, False, False, 0)},
            "dice": None,
...
def test_retained_usage_uses_backend_tokens_and_invalidates_stale_measurements():
    ...
    async def run():
        ...
        assert "weighted text estimate" in manager.usage_snapshot()["counting_method"]
        await manager.refresh_usage()
...
def test_retained_usage_fallback_is_labelled_and_not_clamped():
    ...
    async def run():
        ...
        manager.set_genesis("x" * 40000)
        await manager.refresh_usage()
        snapshot = manager.usage_snapshot()
        assert snapshot["retained_context_tokens"] > manager.context_window_size
        assert "conservative weighted text estimate" in snapshot["counting_method"]
        await manager.close()
