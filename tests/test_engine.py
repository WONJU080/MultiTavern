...
def test_repeated_action_is_annotated_for_definitive_resolution(tmp_path: Path) -> None:
    """A player repeating their exact previous action gets a system annotation."""
    ...
    async def run() -> None:
        ...
        await engine.process_payload("host", payload("action", action="Knock out Player"))
        await engine.process_payload("player", payload("action", action="Resist"))
        await engine.wait_for_inference()
        assert "repeats their exact previous action" not in captured["round"]["Host"]
        assert "repeats their exact previous action" not in captured["round"]["Player"]

        await engine.process_payload("host", payload("action", action="Knock out Player"))
        await engine.process_payload("player", payload("action", action="Watch the door"))
        await engine.wait_for_inference()
        assert "repeats their exact previous action" in captured["round"]["Host"]
        assert "repeats their exact previous action" not in captured["round"]["Player"]
        assert captured["round"]["Host"].endswith("Knock out Player")
...
def test_host_join_rejects_invalid_names(tmp_path: Path) -> None:
    """Reject a missing or oversized host name before any player exists."""
...
def test_title_then_opening_are_the_only_lobby_inference_calls(tmp_path):
    ...
    async def run():
        ...
        assert "original_scenario" not in snapshot
        assert not sender.events_of_type("state_update")
        await engine.process_payload("host", payload("start_game"))
        await engine.wait_for_inference()
        assert len(client.calls) == 2
        prompt = client.calls[1]["messages"][-1]["content"]
        assert "cast of characters" in prompt
        assert "Player (controlled by Player)" in prompt
