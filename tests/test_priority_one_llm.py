...
def test_json_mode_injects_schema_and_parses_manually():
    """Backends without structured outputs use JSON mode and manual validation."""
    settings.llm.provider = "openai"
    settings.llm.structured_outputs = False

    class Completions:
        """Fake chat.completions.create that returns a raw JSON string."""
        ...
        async def create(self, **kwargs):
            ...
            content = json.dumps(
                {
                    "round_title": None,
                    "global_narrative": "The door opens.",
                    "player_resolutions": {"Alice": "Alice steps through."},
                }
            )
            return SimpleNamespace(
                choices=[
    ...
    async def run():
        ...
        result = await manager._parse(
            [{"role": "user", "content": "Act now"}], RoundResolution, "round"
        )
        assert isinstance(result, RoundResolution)
        assert result.global_narrative == "The door opens."
        assert result.player_resolutions == {"Alice": "Alice steps through."}
        request = completions.calls[0]
        assert request["response_format"] == {"type": "json_object"}
        prompt = request["messages"][-1]["content"]
        assert "Return json output matching this schema" in prompt
        assert "global_narrative" in prompt
        assert request["max_completion_tokens"] == settings.llm.round_output_tokens

    asyncio.run(run())


def test_structured_outputs_default_uses_beta_parse():
    """The default path keeps using the OpenAI structured-outputs endpoint."""
    settings.llm.provider = "openai"
    ...
    async def run():
        client = FakeClient()
        manager = LLMContextManager(client)
        await manager._parse([{"role": "user", "content": "Act now"}], RoundResolution, "round")
        assert len(client.calls) == 1
        assert client.calls[0]["response_format"] is RoundResolution

    asyncio.run(run())


def test_dice_planner_prompt_requires_rolls_for_contested_and_repeated_attempts():
    """Contested and repeated attempts are prompted to roll for a definitive outcome."""
    settings.llm.provider = "openai"
    settings.llm.structured_outputs = True
    ...
    async def run():
        client = FakeClient()
        manager = LLMContextManager(client)
        manager.set_genesis("A cabin in the woods.")
        await manager.plan_dice({"Alice": "Knock out Bob", "Bob": "Resist capture"})
        prompt = client.calls[0]["messages"][-1]["content"]
        assert "directly opposes or targets another player" in prompt
        assert "repeats a previously unresolved attempt" in prompt
        assert "Do not let repeated or contested attempts loop without a roll" in prompt
...
def test_unknown_backend_uses_weighted_estimate_and_keeps_small_slot_context():
    """Verify an unknown backend uses the weighted text estimate and keeps a small slot context."""
    ...
    async def run():
        ...
        # 3 CJK chars count as ~3 tokens; the emoji counts as a fraction of one.
        assert manager._count_tokens("💎日本語") == 4
        with pytest.raises(LLMResolutionError):
            await manager._parse(
                [{"role": "user", "content": "漢" * 5000}], RoundResolution, "round"
            )
        assert manager.token_count_method == "conservative weighted text estimate"
        assert not client.calls
        await manager.close()
...
def test_large_next_action_triggers_summary_and_repeated_memory_survives(caplog):
    ...
    async def run():
        ...
        for _ in range(3):
            manager.history = [
                {"role": "user", "content": "Alice examines the gate. " * 400},
                {"role": "assistant", "content": memory().model_dump_json() + " detail" * 560},
            ]
            await manager._request(
                {"role": "user", "content": "Wait. " * 1400}, RoundResolution, remember=True
            )
...
def test_failed_empty_or_expanding_summary_preserves_original(result, caplog):
    ...
    async def run():
        ...
        manager.memory = {"role": "user", "content": memory().model_dump_json()}
        manager.history = [
            {"role": "user", "content": "earlier action " * 800},
            {"role": "assistant", "content": "earlier state " * 500},
        ]
        previous_memory, previous_history = manager.memory, manager.history
        with pytest.raises(LLMResolutionError):
            await manager._request(
                {"role": "user", "content": "next action " * 1150}, RoundResolution, remember=True
            )
...
def test_weighted_estimate_counts_cjk_and_ascii_compactly():
    """Without a tokenizer, CJK chars count ~1 each and ASCII ~4 per token."""
    manager = LLMContextManager(FakeClient())
    assert manager._count_tokens("你好世界") == 4
    assert manager._count_tokens("hello world, this is text!") == 7
    assert manager._count_tokens("") == 1
    assert manager._count_tokens("中文 mixed 123") == 2 + 3


def test_oversized_durable_memory_is_recompressed_within_budget():
    """A durable memory exceeding its budget is recompressed and audited."""
    settings.llm.provider = "openai"
    settings.llm.memory_budget_fraction = 0.3

    def respond(request):
        schema = request["response_format"]
        prompt = request["messages"][-1]["content"]
        if schema is SummaryAudit:
            return SummaryAudit(preserved=True, corrections=[])
        if issubclass(schema, ContextSummary):
            if "Compress the durable memory above" in prompt:
                return ContextSummary(
                    world_state="z " * 60,
                    player_states={"Alice": "Fine."},
                    important_npcs="Mira",
                    unresolved_threads=["Return the key."],
                )
            return ContextSummary(
                world_state="y " * 7000,
                player_states={"Alice": "Broken wrist."},
                important_npcs="Mira",
                unresolved_threads=["Return the key."],
            )
        return RoundResolution(
            global_narrative="The gate opens.",
    ...
    async def run():
        ...
        manager = LLMContextManager(client)
        manager.context_window_size = 10_000
        manager.set_genesis("A gate.")
        ...
        await manager._request(
            {"role": "user", "content": "Wait. " * 4000}, RoundResolution, remember=True
        )
        assert manager.memory is not None
        assert "z " in manager.memory["content"]
        assert manager._context_size([manager.memory]) <= 3000
        recompressions = sum(
            1
            for call in client.calls
            if "Compress the durable memory above" in call["messages"][-1]["content"]
        )
        assert recompressions == 1
        await manager.close()

    asyncio.run(run())


def test_schema_invalid_response_gets_a_targeted_repair_attempt():
    """A schema-invalid response gets one correction retry with the violation detail."""
    settings.llm.provider = "openai"
    settings.llm.structured_outputs = True
