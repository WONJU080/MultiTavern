...
async def auth(engine, who):
    """Create or reconnect a client against the engine."""
    if who in engine.players:
        await engine.player_join(
            who,
            {
                "name": who.title(),
                "character": who.title(),
                "reconnect_token": engine.players[who].reconnect_token,
            },
        )
    elif who == "host":
        await engine.host_join(who, {"name": who.title()})
    else:
        await engine.player_join(who, {"name": who.title(), "character": who.title()})
