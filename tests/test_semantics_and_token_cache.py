...
from core.schemas import Character, DicePlan, RoundResolution, ScenarioTitle, SummaryAudit
...
def test_validated_raw_response_retains_spacing_but_normalization_is_consistent():
    ...
    async def run():
        ...
        manager = LLMContextManager(client)
        # A realistic deployment window: this test covers spacing, not the budget.
        manager.context_window_size = 65_536
...
def test_message_cache_matches_fresh_counts_and_stays_bounded():
    ...
    expected = 32 + (3 + 32) + (2 + 32)  # 3 CJK chars ≈ 3 tokens; "A gate" ≈ 2 tokens
